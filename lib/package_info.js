...
/**
 * Read servicedoc.json / package.json data
 */
PackageInfo.prototype.get = function() {
    ...
    if (packageJson.servicedoc)
        result = packageJson.servicedoc;
    ...
    // read servicedoc.json (and overwrite package.json information)
    var servicedocJson = this._readPackageData('servicedoc.json');

    // servicedoc.json has higher priority
    _.extend(result, servicedocJson);
    ...
    if (Object.keys(servicedocJson).length === 0 && ! packageJson.servicedoc)
        app.log.warn('Please create an servicedoc.json configuration file.');
