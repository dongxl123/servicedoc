...
Parser.prototype.parseFile = function (filename, encoding) {
    ...
    // https://github.com/servicedoc/servicedoc-core/pull/25
        ...
        for (var j = 0; j < elements.length; j += 1) {
            ...
            if (!elementParser) {
                ...
                if (preventGlobal) {
                    ...
                    // (e.g. @successTitle is global, but should co-exist with @serviceErrorStructure)
 ...
 *
 * An @serviceIgnore ignores the block.
 * Other, non @service elements, will be ignored.
 */
Parser.prototype._findBlockWithApiGetIndex = function (blocks) {
    ...
    for (var i = 0; i < blocks.length; i += 1) {
        ...
        for (var j = 0; j < blocks[i].length; j += 1) {
            // check apiIgnore
            if (blocks[i][j].name.substr(0, 9) === 'serviceignore') {
                app.log.debug('serviceIgnore found in block: ' + i);
                found = false;
                break;
            }

            // check app.options.apiprivate and apiPrivate
            if (!app.options.apiprivate && blocks[i][j].name.substr(0, 10) === 'serviceprivate') {
                app.log.debug('private flag is set to false and servicePrivate found in block: ' + i);
                found = false;
                break;
            }

            if (blocks[i][j].name.substr(0, 3) === 'api' || blocks[i][j].name.substr(0, 7) === 'service')
                found = true;
        }
        if (found) {
            foundIndexes.push(i);
            app.log.debug('service found in block: ' + i);
        }
    }
    return foundIndexes;
};
