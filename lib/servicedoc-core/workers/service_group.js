...
var _messages = {
    common: {
        element: 'apiGroup',
        usage  : '@serviceGroup group',
        example: '@serviceDefine MyValidGroup Some title\n@serviceGroup MyValidGroup'
    }
};
...
function postProcess(parsedFiles, filenames, preProcess, packageInfos, source, target, messages) {
    ...
    parsedFiles.forEach(function(parsedFile, parsedFileIndex) {
        parsedFile.forEach(function(block) {
            ...
            // could overwrite local names on a later starting worker process from e.g. @serviceUse
...
/*
                ...
                throw new WorkerError('Referenced groupname does not exist / it is not defined with @serviceDefine.',
