...
function PluginLoader(_app) {
  ...
  // Try to load global servicedoc-plugins (if servicedoc is installed locally it tries only local)
  this.detectPugins(__dirname);

  // Try to load local servicedoc-plugins
...
/**
 * Detect modules start with "servicedoc-plugin-".
 * Search up to root until found a plugin.
 */
PluginLoader.prototype.detectPugins = function(dir) {
  var self = this;

  // Every dir start with "servicedoc-plugin-", because for the tests of servicedoc-plugin-test.
  var plugins;
  try {
    plugins = glob.sync(dir + '/servicedoc-plugin-*')
    .concat( glob.sync(dir + '/@*/servicedoc-plugin-*') );
  } catch (e) {
    app.log.warn(e);
    return;
  }
